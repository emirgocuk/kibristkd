...
router.get("/", async (_req, res) => {
  ...
  const list = await repo.find({ where: { status: "published" as any }, order: { createdAt: "DESC" } });
...
router.get("/:id", async (req, res) => {
  ...
  const m = await repo.findOne({ where: { id, status: "published" as any } });
...
router.put("/:id", requireAuth, async (req: AuthedRequest, res) => {
  ...
  if (b.baslik ?? b.title) patch.baslik = String(b.baslik ?? b.title);
