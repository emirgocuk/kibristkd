...
import { AuthedRequest, requireAuth, requireAdmin } from "../middleware/authMiddleware.js";
...
// Multer'lı istek için tip
type MulterAuthedRequest = AuthedRequest & { file?: Express.Multer.File };
...
router.get("/slug/:slug", async (req, res) => {
  ...
  const m = await repo.findOne({
    where: { slug: req.params.slug, status: "published" as any },
  });
  if (!m) return res.status(404).json({ success: false, message: "Makale bulunamadı" });
...
router.post("/", requireAuth, async (req: AuthedRequest, res) => {
  try {
    ...
    if (req.user?.role === "admin" && b.status) {
      ...
      if (["draft", "pending", "published"].includes(s)) status = s as any;
    }
...
router.put("/:id", requireAuth, async (req: AuthedRequest, res) => {
  ...
  if (b.icerik ?? b.content ?? b.body) patch.icerik = String(b.icerik ?? b.content ?? b.body);
  if ("kategori" in b || "category" in b) patch.kategori = (b.kategori ?? b.category ?? "").toString().trim() || null;
  if ("kapakResmi" in b || "image" in b) patch.kapakResmi = (b.kapakResmi ?? b.image ?? "").toString().trim() || null;

  if (isAdmin && b.status) {
    const s = String(b.status).toLowerCase();
    if (["draft", "pending", "published"].includes(s)) (patch as any).status = s;
  } else {
...
router.get("/:id/kapak", async (req, res) => {
  ...
  if (!m || !m.kapakResmiBlob) return res.status(404).send("Bulunamadı");
