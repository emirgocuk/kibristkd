            ...
            <List disablePadding>
                {recentNews.map((news, index) => (
                    <ListItem key={index} disablePadding sx={{ mb: 2 }}>
                        <ListItemAvatar sx={{ minWidth: '60px' }}><Avatar variant="square" src={logo} /></ListItemAvatar>
                        <ListItemText primary={<MuiLink component={Link} to={news.path} color="inherit" underline="hover">{news.title}</MuiLink>} secondary={news.date} secondaryTypographyProps={{ color: 'grey.500' }}/>
                    </ListItem>
                ))}
            </List>
          </Grid>
